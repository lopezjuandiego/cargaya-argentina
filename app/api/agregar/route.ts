...
export async function POST(req: NextRequest) {
  try {
    ...
    if (typeof name !== "string" || name.length > MAX.name) return NextResponse.json({ error: "Nombre demasiado largo" }, { status: 400 });
    if (address.length > MAX.address) return NextResponse.json({ error: "Dirección demasiado larga" }, { status: 400 });
    if (city.length > MAX.city) return NextResponse.json({ error: "Ciudad demasiado larga" }, { status: 400 });
    if (comment && typeof comment === "string" && comment.length > MAX.comment) return NextResponse.json({ error: "Comentario demasiado largo (máx. 1000 caracteres)" }, { status: 400 });
    if (!Array.isArray(connectorTypes ?? [])) return NextResponse.json({ error: "Conectores inválidos" }, { status: 400 });

    const hasCoordsInput = lat !== undefined && lat !== null && lng !== undefined && lng !== null;
